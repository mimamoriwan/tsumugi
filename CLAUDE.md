...
### 優先度高
1. **メモの編集・削除** → 一覧からクリックして全文表示・編集・削除

### 優先度中
3. **タグクリックで絞り込み** → タグを押すと同タグのメモ一覧
4. **Vercel Edge Functions** → 本番環境でもAI機能を安全に動かす

### 優先度低（将来）
5. **Capacitorでネイティブ化** → Xcode経由でApp Store申請
6. **マルチデバイス最適化** → iPad・iPhoneでの表示調整
